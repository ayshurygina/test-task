patterns:
    ...
    $createInf = (создать/установить/сделать/издать/ввести/поставить)
    ...
    $pin = (пин/pin/(пин/pin) (~код/кот)/пинкод*/пинкот*/pin cod*/pincod*/pinкод*/pinкот*)
    ...
    $appGen = ([$regex<моб\.>/мобильн*] приложения/(моб/мобильн*) банка)
    $appDat = ([моб/мобильн*] приложению/(моб/мобильн*) банку)
    
    $toOpenApp = ({([для/от] (входа [в]/запуска/открытия/разблокировки)) $app}
        | {((при/на) (входе [в]/запуске/открытии)) $app}
        | {([в/во] (вход/запуск/открытие)) ((в/во) $app)})
